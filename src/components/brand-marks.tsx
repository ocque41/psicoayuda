...
type VenezuelaFlagProps = {
  className?: string;
  decorative?: boolean;
};

/** Bandera de Venezuela con el arco correcto de ocho estrellas. */
export function VenezuelaFlag({
  ...
  return (
    <svg
      ...
      aria-label={decorative ? undefined : "Bandera de Venezuela"}
    >
      {!decorative && <title>Bandera de Venezuela</title>}
...
/** Firma secundaria de Nido. La bandera de Venezuela conserva la prioridad. */
export function NidoLogo({
  className,
  decorative = false,
  priority = false,
}: NidoLogoProps) {
